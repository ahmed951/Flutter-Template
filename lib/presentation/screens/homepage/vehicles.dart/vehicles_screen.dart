...
class VehiclesScreen extends StatelessWidget {
  ...
  final MenuItems menu = MenuItems(items: [
    const MenuItem(text: 'Edit'),
    const MenuItem(text: 'Delete'),
  ]);
          ...
          body: SingleChildScrollView(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  const DefaultBreadCrumb(items: ["Home", "Vehicles"]),
                  vertialSpace(3.h),
                  Text(
                    "Vehicles".tr(),
                    style:
                    ...
                    children: [
                      Expanded(
                        child: CupertinoSearchTextField(
                          onChanged: (e) {
                            cubit.searchVehicles();
                          },
                          controller: cubit.vehiclesSearchController,
                  ...
                  defaultButtonWithIcon(
                      text: "Add Vehicle".tr(),
                      func: () {
                        pushNavigate(context, addVehicleRoute, arguments: {
                          "context": context,
                          "isEdit": false
                        }).then((value) => cubit.getVehicles());
                      },
                      icon: Icons.add),
                  vertialSpace(3.h),
                  if (cubit.vehiclesModel != null)
                    ListView.separated(
                      shrinkWrap: true,
                      physics: const NeverScrollableScrollPhysics(),
                      itemCount: cubit.vehiclesSearchController.text.isEmpty
                          ? cubit.vehiclesList.length
                          : cubit.searchVehiclesList.length,
                      separatorBuilder: (BuildContext context, int index) {
                        return vertialSpace(2.h);
                      },
                      itemBuilder: (BuildContext context, int index) {
                        var data = cubit.vehiclesSearchController.text.isEmpty
                            ? cubit.vehiclesList[index]
                            : cubit.searchVehiclesList[index];
                        return Card(
                          elevation: 5,
